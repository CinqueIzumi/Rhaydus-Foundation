        ...
        return withContext(Dispatchers.Main) {
            ...
            suspendCancellableCoroutine<ShareOutcome> { continuation ->
                ...
                activityController.completionWithItemsHandler = { _, completed, _, _ ->
                    continuation.resume(if (completed) ShareOutcome.Shared else ShareOutcome.Cancelled)
                }
                ...
                // If the caller's coroutine is cancelled while the sheet is still up (e.g. the screen is
                // torn down mid-share), dismiss it on the main queue so no orphaned controller lingers.
                continuation.invokeOnCancellation {
                    dispatch_async(dispatch_get_main_queue()) {
                        activityController.dismissViewControllerAnimated(
                            flag = true,
                            completion = null,
                        )
                    }
