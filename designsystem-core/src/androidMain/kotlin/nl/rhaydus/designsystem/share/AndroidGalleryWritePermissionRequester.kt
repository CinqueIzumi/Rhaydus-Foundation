...
/**
 * Compose-aware gate for the legacy `WRITE_EXTERNAL_STORAGE` permission needed when writing to the gallery
 * on API ≤ 28. On API 29+ the permission is unnecessary (scoped storage handles it), so [request] fires
 * `onResult` synchronously with `granted = true`.
 */
