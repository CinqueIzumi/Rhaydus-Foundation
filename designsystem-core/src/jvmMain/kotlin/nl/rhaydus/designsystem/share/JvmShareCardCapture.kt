...
/**
 * Desktop share-card capture. Compose Desktop renders through Skiko, so the capture path mirrors the iOS
 * actual: the [GraphicsLayer] is read on the render thread and encoded to PNG via Skia. Desktop has no
 * system share sheet, so [share] copies the image to the system clipboard (the clean desktop equivalent —
 * the user pastes it wherever they like); [saveToGallery] opens a native save dialog.
 */
internal class JvmShareCardCapture internal constructor(
    ...
    override suspend fun saveToGallery(displayName: String): SaveOutcome {
        ...
        val target = withContext(Dispatchers.Main) { chooseSaveFile(displayName) }
        ...
        return withContext(Dispatchers.IO) {
            runCatching {
                val directory = File(
                    ...
                    "${config.fileNamePrefix}-$SHARE_CACHE_FOLDER",
                )
                directory.mkdirs()
    ...
    private suspend fun capturePngBytes(): ByteArray? {
        val skiaBitmap = withContext(Dispatchers.Main) {
    ...
    private fun buildFilename(displayName: String): String {
        ...
        return "${config.fileNamePrefix}-$sanitized.png"
    }
...
actual fun rememberShareCardCapture(config: ShareCardCaptureConfig): ShareCardCapture {
    ...
    return remember(graphicsLayer, config) {
        JvmShareCardCapture(
